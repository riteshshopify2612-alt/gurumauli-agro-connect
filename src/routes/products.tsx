...
import { PRODUCTS, whatsappOrderUrl } from "@/lib/site-data";
...
export const Route = createFileRoute("/products")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse Samtol Jeevan grains, fertilizers, seeds, organic products and crop nutrition solutions from Gurumauli Agro Agency." },
      { property: "og:title", content: "Products — Gurumauli Agro Agency" },
      { property: "og:description", content: "Quality fertilizers, seeds, grains and agro products for every farmer." },
    ],
  }),
    ...
    <SiteLayout>
      <section className="bg-gradient-to-b from-secondary/60 to-background py-16">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-muted-foreground">Click any product to view details and order on WhatsApp.</p>
        </div>
      </section>
            ...
            <div className="p-6">
              ...
              <div className="mt-5 flex gap-2">
                <Link
                  to="/products/$slug"
                  params={{ slug: p.slug }}
                  className="flex-1 inline-flex items-center justify-center gap-1 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary-glow"
                >
                  View Details <ArrowRight className="h-4 w-4" />
                </Link>
                <a
                  href={whatsappOrderUrl(p.title)}
                  target="_blank"
                  rel="noopener"
                  className="inline-flex items-center justify-center rounded-full bg-whatsapp px-4 py-2 text-sm font-semibold text-white"
